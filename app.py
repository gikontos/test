...
def book_page(arguement):
    ...
    result = cur.fetchone()
    return render_template("book_page.html", isbn=arguement, title=result[0], keywords=result[1], summary=result[2],
                           no_pages=result[3], publisher=result[4], image=result[5])
...
def user_page(arguement):
    ...
    if "reserve" in request.form:
        ...
        cur = mysql.connection.cursor()
        try:
            cur.execute("insert into reservations (user_id,book_id) values (%s,%s)",
                        (arguement, session["available_isbns"][int(pressed_button)]))
    ...
    if "form2" in request.form:
        ...
        cur.close()
        return render_template("user_dashboard.html", username=arguement, available_books=available_titles,
                               borrowed_books=borrowed_titles, writer2=writer, category2=category, title2=title,
                               title1=title1, category1=category1, writer1=writer1, available_isbns=available_isbns)
    if "form1" in request.form:
        ...
        cur.close()
        return render_template("user_dashboard.html", username=arguement, available_books=available_titles,
                               borrowed_books=borrowed_titles, writer1=writer, category1=category, title1=title,
                               title2=title2, category2=category2, writer2=writer2, available_isbns=available_isbns)

    cur = mysql.connection.cursor()
    cur.execute(
        "select  title,isbn from users join schools on users.school_id=schools.school_id join schools_books on schools.school_id=schools_books.school_id join books on schools_books.book_id=isbn where users.username=%s",
        (arguement,))
    result = cur.fetchall()
    available_titles = [row[0] for row in result]
    available_isbns = [row[1] for row in result]
    cur.execute(
        "select  title,isbn from users join borrowings on user_id=username join books on book_id=isbn where username=%s",
        (arguement,))
    result = cur.fetchall()
    borrowed_titles = [row[0] for row in result]
    borrowed_isbns = [row[1] for row in result]
    ...
    return render_template("user_dashboard.html", username=arguement, available_books=available_titles,
                           borrowed_books=borrowed_titles, available_isbns=available_isbns)
...
def operator_dashboard(arguement):
    ...
    if request.method == "POST":
        ...
        # if no_copies != "":
            # no_copies_check = " and "
        cur = mysql.connection.cursor()
        cur.execute("select title, first_name,last_name from books join book_writer on books.isbn=book_writer.book_id join writers on book_writer.writer_id=writers.id where users.username=%s "+title_check+category_check+writer_check,param)
        result = cur.fetchall()
        cur.close
